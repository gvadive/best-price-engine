...
public class OrderService {
    ...
    public Order placeOrder(OrderRequest request) {
        ...
        return request.bulkPricingRequestId() != null
                ? placeOrderAgainstBulkPricing(offer, request)
                : placeOrderAgainstLivePrice(offer, request);
    }

    /** Used by PriceWatchOrderService once a watch's price condition is satisfied -- fills at the offer's current price. */
    @Transactional
    public Order fulfillAtLivePrice(Offer offer, int quantity, FulfillmentMode mode) {
        return fulfill(offer, quantity, offer.getPriceVersion(), mode, offer.unitPriceAt(quantity));
    }

    /** Standard path: caller quotes the offer's live priceVersion; a mismatch means the price moved. */
    private Order placeOrderAgainstLivePrice(Offer offer, OrderRequest request) {
        if (offer.getPriceVersion() != request.quotedPriceVersion()) {
            return orderRepository.save(
                    Order.rejectedForStalePrice(offer, request.quantity(), request.quotedPriceVersion()));
        }
        return fulfill(offer, request.quantity(), request.quotedPriceVersion(),
                request.fulfillmentMode(), offer.unitPriceAt(request.quantity()));
    }

    /** Bulk-pricing path: caller quotes a specific negotiated BulkPricingRequest instead of the live price. */
    private Order placeOrderAgainstBulkPricing(Offer offer, OrderRequest request) {
        ...
        if (invalidReason != null) {
            return orderRepository.save(Order.rejectedForInvalidQuote(offer, request.quantity(), invalidReason));
        }

        Order order = fulfill(offer, request.quantity(), 0, request.fulfillmentMode(), quote.getNegotiatedUnitPrice());
     ...
     */
    private Order fulfill(Offer offer, int quantity, long quotedPriceVersion, FulfillmentMode mode, double unitPrice) {
        Order order;
        if (mode == FulfillmentMode.EXACT_QUANTITY) {
            if (offer.getAvailableQuantity() >= quantity) {
                offer.reduceStock(quantity);
                order = Order.filledAt(offer, quantity, quotedPriceVersion, quantity, unitPrice);
            } else {
                order = Order.rejectedForInsufficientStock(offer, quantity, quotedPriceVersion);
            }
        } else {
            int filled = offer.reduceStock(quantity);
            order = Order.filledAt(offer, quantity, quotedPriceVersion, filled, unitPrice);
        }
